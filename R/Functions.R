...
calcBetaDifference <- function(matrix3) {
  ...
  matrix3 <-
    plyr::rbind.fill(matrix3, bd_UPvsMID, bd_UPvsDOWN, bd_MIDvsDOWN)
...
Analysis <- function(matrix1) {
  medianUP <-
    apply(as.data.frame(matrix1[which(matrix1$stratification %in% "UP"), -ncol(matrix1)]), 2, median, na.rm = T)
  medianMID <-
    apply(as.data.frame(matrix1[which(matrix1$stratification %in% "Medium"), -ncol(matrix1)]), 2, median, na.rm = T)
  medianDOWN <-
    apply(as.data.frame(matrix1[which(matrix1$stratification %in% "Down"), -ncol(matrix1)]), 2, median, na.rm = T)
  ...
  dfTtest <-
    data.table::setnames(data.frame(matrix(nrow = 3, ncol = dimM)), colnamesDfTtest)
...
AnalysisIslands_PositionsCG <-
  function(leng,
    ...
    for (k in 1:leng) {
      cg <-
        as.vector(dfCGunique[which(dfCGunique$gene %in% genes[index] &
      ...
      if (length(cg) != 0) {
        ...
        tempMatrix2 <-
          as.data.frame(tempMatrix2[, colSums(is.na(tempMatrix2)) != nrow(tempMatrix2)]) #remove all coloumns that have all values NA
        ...
        if (dim(tempMatrix2)[2] > 1) {
          ...
          mTmp <-
            as.data.frame(rep(dfPancan2[c(1:473), genes[index]], length(cg)))
          mTmp1 <- as.data.frame(tempMatrix2[1:dim(mTmp)[1],])
          resultCorrTest <-
            psych::corr.test(mTmp, mTmp1, adjust = "none")
          tempMatrix2 <-
            rbind(tempMatrix2,
                  as.numeric(resultCorrTest$r),
                  as.numeric(resultCorrTest$p)) #add pearson correlation and p-value
          #[END]

          tempMatrix2 <-
            data.frame(sapply(tempMatrix2, c, unlist(valExprGene[, genes[index]])), row.names = NULL)

          tempMatrix2 <- as.data.frame(tempMatrix2[-c(1:num_row), ])
          colnames(tempMatrix2) <-
            paste(position[k], genes[index], sep = "_")
...
calculateTtest <- function(array1, array2, flag) {
  ...
  if (flag) {
    ...
    if (difference != 0) {
      A <-
        t.test(array1, array2, var.equal = F)[c('statistic', 'p.value')]
...
setRowNames <- function(df) {
  rowNames <-
    c(
      ...
      "fc_MIDvsDOWN(gene)",
      "pvalue_UPvsMID(gene)",
      "pvalue_UPvsDOWN(gene)",
      "pvalue_MIDvsDOWN(gene)"
    )
