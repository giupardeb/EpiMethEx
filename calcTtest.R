...
#for(i in 2:1) {
  ...
  rowGene = strsplit(line[2], "\t")[[1]] #contienetutti i valori del gene ordinati in modo decrescente
  ...
  valuesGene = unlist(listsGene[1])
    ...
    #second_percentile = quantile(valuesGene[1], 0.66)
    #third_percentile = quantile(valuesGene[1], 0.99)
    
  #  div_array = split(valuesGene[1], ceiling(seq_along(valuesGene)/(ceiling(lengthArray/3))))
  # 
#   #remove(valuesGene)
# 
